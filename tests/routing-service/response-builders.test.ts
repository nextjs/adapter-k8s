...
import { buildImmediateResponse, buildHeaderMutationResponse } from '../../src/routing-service/response-builders.js';
...
describe('buildImmediateResponse', () => {
  it('builds a redirect response', () => {
    ...
    const locationHeader = resp.immediateResponse!.headers!.setHeaders!.find(h => h.header.key === 'location');
    expect(locationHeader).toBeDefined();
    expect(locationHeader!.header.value).toBe('https://example.com/new');
  });

  it('builds a response with body', () => {
    const resp = buildImmediateResponse(502, { 'content-type': 'text/plain' }, 'External rewrites not supported');
