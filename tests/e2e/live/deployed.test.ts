// Live end-to-end tests against a DEPLOYED instance of the flagship fixture app
// (see fixtures/main/). These hit a real URL over the network — they are
// intentionally NOT part of the default `vitest run` (excluded via config). Run with:
//
  ...
  try {
    ...
    throw new Error(
      `Deployment not reachable at ${BASE} (${(err as Error).message}). ` +
        `Deploy the fixtures/main app or set E2E_BASE_URL.`,
    );
  }
});
