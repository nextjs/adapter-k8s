...
if [ ! -f config/routing-manifest.json ]; then
  node - "${BUILD_ID}" <<'NODE'
const { writeFileSync } = require('node:fs')
writeFileSync('config/routing-manifest.json', JSON.stringify({
  routeGraph: { beforeMiddleware: [], beforeFiles: [], afterFiles: [], dynamicRoutes: [],
    onMatch: [], fallback: [], shouldNormalizeNextData: false, rsc: {} },
  pathnames: [], i18n: null, buildId: process.argv[2], basePath: '', middleware: null,
  poolAssignments: {}, pprRoutes: {}, nextVersion: require('next/package.json').version,
}))
NODE
fi
