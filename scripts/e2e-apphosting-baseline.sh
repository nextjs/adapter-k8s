...
WORKSPACE="${ADAPTER_DIR}/../.adapter-k8s-e2e-apphosting"
NEXTJS_DIR="${NEXTJS_DIR:-${WORKSPACE}/next.js}"
TEST_PATTERN="${1:-}"
NEXTJS_REF="${2:-canary}"
TEST_RETRIES="${NEXT_TEST_RETRIES:-0}"
BASELINE_MANIFEST="${ADAPTER_DIR}/test/deploy-tests-manifest.apphosting-baseline.json"
NEXT_TEST_FILTERS="test/deploy-tests-manifest.json,${BASELINE_MANIFEST}"

echo "=== Apphosting Adapter Baseline E2E Runner ==="
echo "Adapter baseline: @apphosting/adapter-nextjs@${APPHOSTING_BASELINE_ADAPTER_VERSION:-latest}"
...
cd "$ADAPTER_DIR"
chmod +x scripts/e2e-baseline-apphosting-deploy.sh scripts/e2e-logs.sh scripts/e2e-cleanup.sh
...
export NEXT_TEST_DEPLOY_SCRIPT_PATH="${ADAPTER_DIR}/scripts/e2e-baseline-apphosting-deploy.sh"
