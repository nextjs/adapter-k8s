...
- **Time.** The proof also binds its mint time, and a pool refuses one older than 120 s (or dated more than 30 s in the future). Without that, a captured trusted exchange could be re-sent verbatim indefinitely — the tuple never changes. The window is anchored on the 60 s per-hop time-to-response-head budget, so it covers an in-flight request across the edge and one cross-pool hop and nothing more. Rejection is the usual fail-safe: the dispatch headers are stripped and the pool re-resolves locally.
...
A pool that refuses a presented proof — for any of these reasons — counts it on `adapter_k8s.pool.dispatch_proof.rejected{reason}` and logs it (throttled per reason). Trusted dispatch silently never engaging is a real failure mode, and it is meant to be visible.
