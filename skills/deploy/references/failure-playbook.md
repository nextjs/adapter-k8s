...
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Deployment <name> did not finish rolling out within <n>s`         | New pool pods never became Ready (image pull, crash loop, scheduling). `<n>` is the derived `kubectl rollout status` budget: replica count x 225s of serial surge cost (ready + `minReadySeconds` + 120s preStop + the pool's 60s post-SIGTERM drain), floored at 600s and capped at 1800s | `kubectl logs deployment/<name> -n <ns> --tail=40`; `kubectl describe pod` for pull/scheduling events; fix, re-run deploy. Pods **Pending** with `didn't match Pod's node affinity/selector` on every node = platform mismatch — the chart stamps `kubernetes.io/arch` from the build's target platform (default `linux/amd64`); rebuild with `ADAPTER_K8S_TARGET_PLATFORM=linux/arm64` for arm64 nodes |
| `Routing service (...) did not become healthy`                     | The ext_proc edge image cannot roll out — a broken routing image would otherwise serve stale edge code silently                                                                                                                                                                            | `kubectl logs -l app.kubernetes.io/component=routing-service -n <ns> --tail=40`                                                                                                                                                                                                                                                                                                                         |
