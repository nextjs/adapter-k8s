...
| ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Deployment <name> did not finish rolling out within 120s`         | New pool pods never became Ready (image pull, crash loop, scheduling). The actual wait is `kubectl rollout status --timeout=600s`; "120s" in the message is stale text | `kubectl logs deployment/<name> -n <ns> --tail=40`; `kubectl describe pod` for pull/scheduling events; fix, re-run deploy. Pods **Pending** with `didn't match Pod's node affinity/selector` on every node = platform mismatch — the chart stamps `kubernetes.io/arch` from the build's target platform (default `linux/amd64`); rebuild with `ADAPTER_K8S_TARGET_PLATFORM=linux/arm64` for arm64 nodes |
