...
export function generateHelmChart({
  ...
  files['Chart.yaml'] = renderChartYaml({ name: releaseName, version: `0.1.0+${buildId}` });
  files['values.yaml'] = renderValuesYaml({ pools, buildId, nextVersion, config, imageRegistry });
  ...
  for (const [poolName, pool] of pools) {
    const scaling = pool.config.scaling ?? { min: 1, max: 3, targetCPU: 80 };
