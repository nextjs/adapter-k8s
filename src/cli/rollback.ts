...
export async function runRollback(options: { projectDir: string; releaseName: string; dryRun?: boolean }): Promise<void> {
  ...
  const currentLower = currentBuildId.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 10);
  const previousLower = previousBuildId.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 10);
  ...
  if (dryRun) {
    ...
    console.log(`  [dry-run] Would swap state: buildId=${previousBuildId}, previousBuildId=${currentBuildId}`);
    return;
  }
  ...
  await execCapture("kubectl", [
    "rollout", "status", `deployment/${previousDeploy}`, "--timeout=120s",
  ]);
    ...
    if (infra.projectId) {
      ...
      for (let i = 0; i < 24; i++) {
        const bsResult = await execCapture("gcloud", [
          ...
          "--project", infra.projectId, "--global",
          "--filter", `name~${releaseName} AND name~${previousLower}`,
        ...
        if (bsName) {
          const healthResult = await execCapture("gcloud", [
            ...
            "--project", infra.projectId, "--global", "--format=json",
          ]);
          if (healthResult.exitCode === 0) {
            try {
              const data = JSON.parse(healthResult.stdout);
              const healthyCount = data.reduce((n: number, b: any) =>
                n + (b.status?.healthStatus ?? []).filter((h: any) => h.healthState === "HEALTHY").length, 0
              );
              if (healthyCount > 0) {
  ...
  const safePreviousBuild = previousBuildId.toLowerCase().replace(/[^a-z0-9.-]/g, '-').replace(/^-+|-+$/g, '').slice(0, 63);

  // Find pool names from active services
  const activeSvcsResult = await execCapture("kubectl", [
    "get", "services", "-l", `app.kubernetes.io/managed-by=adapter-k8s-active,app.kubernetes.io/name=${releaseName}`,
  ...
  if (activeSvcsResult.exitCode === 0 && activeSvcsResult.stdout.trim()) {
    ...
    for (const svcName of activeSvcsResult.stdout.trim().split("\n")) {
      if (!svcName) continue;
      await execCapture("kubectl", [
        "patch", "service", svcName, "--type=json",
