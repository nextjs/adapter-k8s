...
`;
  ...
  // The README must describe the bundle that was ACTUALLY emitted: a job-mode bundle
  // whose README hands the operator mode-none's out-of-band cutover instructions sends
  // them to patch selectors the Job owns. Mode none keeps its text byte-identical.
  const cutoverMode = meta.cutover ?? "none";
  const cutoverSection =
    cutoverMode === "job"
...
Generated by \`adapter-k8s emit\` (cutover.mode: ${cutoverMode}). Replace this directory WHOLESALE
