...
`test:package` packs with lifecycle scripts disabled, extracts the tarball, checks the public file set, imports the adapter, loads the CLI, and verifies that `createRequire().resolve()` can locate the adapter entrypoint used by `adapterPath`.
...
2. Merge the release stack. Create and publish a GitHub Release whose tag is exactly `v<package.json version>`; the workflow refuses a mismatch.
3. Configure npm trusted publishing for `nextjs/adapter-k8s` and workflow `release.yml`. It uses a GitHub-hosted runner, `id-token: write`, a pinned OIDC-capable npm CLI, public access, and provenance.
