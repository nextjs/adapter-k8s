...
An existing instance is reused only when its size, tier, VPC network, connect mode, and AUTH/TLS posture match the plan. Changing the managed-cache project or region in place is rejected because the current lifecycle records one paid instance and cannot yet replace it without orphaning the old one. Destroy and recreate intentionally for those coordinate changes.
