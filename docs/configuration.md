...
An existing instance is reused only when its size, tier, canonical VPC project/name, connect mode, and AUTH/TLS posture match the plan. Before the create call, deploy records the intended cache project and region with a pending marker; this remains recoverable even if the cloud operation times out or later credential retrieval fails. A successful retry clears the pending marker. Never edit only one cache coordinate in `infrastructure.json`: deploy and destroy reject incomplete identities instead of guessing which project owns paid state.
